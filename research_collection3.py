...
#     # YouTube Search
#     youtube_results = await youtube_agent_chain.run(f"{topic}")
...
#     semantic_scholar_response = await semantic_scholar_executor.invoke({"input": semantic_query})
...
#     wiki_results = await wiki_agent(wiki_search_query)
...
# Async function to collect and structure results
async def collect_and_format_resources(topic):
    collected_data = ""

    # YouTube Agent
    youtube_results = await youtube_agent_chain.ainvoke(f"{topic}")
    st.write("got YouTube results")
    collected_data += f"### YouTube Results:\n{youtube_results}\n\n"

    # Semantic Scholar Agent
    semantic_query = f"query: {topic}\nFor each relevant paper, create 3 sections: paper name with URL, summary, and citations."
    semantic_scholar_response = await semantic_scholar_executor.ainvoke({"input": semantic_query})
    st.write("got Semantic Scholar Results")
    collected_data += f"### Semantic Scholar Results:\n{semantic_scholar_response}\n\n"

    # Wikipedia Agent
    wiki_results = await wiki_agent.ainvoke(topic)
    st.write("got Wiki Articles Summary")
    collected_data += f"### Wikipedia Articles Summary:\n{wiki_results}\n\n"

    # Final structuring
    final_prompt = (
        "You are an expert in summarizing and organizing research information. "
        "You will be given data collected from various sources, such as YouTube videos, academic papers and Wikipedia article summaries. "
        "Please structure this data into a well-organized, readable report with the following sections:\n\n"
        "1. **YouTube Videos**: A list of the top videos related to the topic with brief descriptions.\n"
        "2. **Academic Papers (Semantic Scholar)**: A list of papers, their summaries, and key citations for each paper.\n"
        "3. **Wikipedia Articles**: A summary of the relevant Wikipedia articles.\n\n"
        "Here is the data to organize:\n\n"
        f"{collected_data}\n"
        "Please ensure the final output is well-structured and easy to understand."
    ...
    structured_response = await llm.ainvoke(final_prompt)
    return structured_response
